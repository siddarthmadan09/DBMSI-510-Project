...
/**
 * A IntervalTPage is a leaf page on a interval-tree.  It holds abstract 
 * <key, RID> pairs; it doesn't know anything about the keys 
 * (their lengths or their types), instead relying on the abstract
 * interface consisting of IntervalT.java.
 */
public class IntervalTPage extends IntervalTSortedPage {
  
  /** pin the page with pageno, and get the corresponding IntervalTPage,
   * also it sets the type to be NodeType.NODE.
   *@param pageno Input parameter. To specify which page number the
   *  IntervalTPage will correspond to.
   *@param keyType AttrType.attrInterval only.
   *    Input parameter.   
   *@exception IOException  error from the lower layer
   *@exception ConstructPageException IntervalTPage constructor error
   */
  public IntervalTPage(PageId pageno, int keyType) 
    throws IOException, 
  ...
  /**associate the IntervalTPage instance with the Page instance,
   * also it sets the type to be NodeType.NODE. 
  ...
  /**new a page, associate the IntervalTPage instance with the Page instance,
   * also it sets the type to be NodeType.NODE. 
   *@param keyType AttrType.attrInterval only.
   *  Input parameter.
   *@exception IOException  error from the lower layer
   ...
   * which  provide an iterator interface to the records on a IntervalTPage.
  ...
  public KeyDataEntry getFirst(RID rid) 
    ...
    * iterator interface to the records on a IntervalTPage.
   ...
   */
  boolean redistribute(IntervalTPage leafPage, IntervalTIndexPage parentIndexPage, 
