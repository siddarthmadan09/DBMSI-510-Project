 ...
 */
public class IntervalTreeFile extends IndexFile 
  ...
  private IntervalTHeaderPage headerPage;
   ...
   *          of this btree file.
   */
  public IntervalTHeaderPage getHeaderPage() {
    return headerPage;
  }
  ...
  public IntervalTreeFile(String filename)
    ...
    {      
      ...
      headerPage= new  IntervalTHeaderPage( headerPageId);       
   ...
   */
  public IntervalTreeFile(String filename, int keytype,
	...
	{
	  headerPage= new  IntervalTHeaderPage(); 
  ...
  private void  updateHeader(PageId newRoot)
	     ...
	     UnpinPageException
    {
      ...
      IntervalTHeaderPage header;
      PageId old_data;
      
      
      header= new IntervalTHeaderPage( pinPage(headerPageId));
  ...
  private KeyDataEntry  _insert(KeyClass key, RID rid,  
	...
	IntervalTIndexPage  currentIndexPage=new IntervalTIndexPage(page, 
	...
	currentIndexPage= new  IntervalTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    IntervalT.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	newIndexPage= new IntervalTIndexPage(headerPage.get_keyType());
	...
	if (IntervalT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
      ...
      else if ( currentPage.getType()==NodeType.NODE)
	{
	  IntervalTPage currentLeafPage = 
	    new IntervalTPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      IntervalT.getKeyDataLength(key, NodeType.NODE) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
		  IntervalTPage rightPage;
	      rightPage=new IntervalTPage(rightPageId, headerPage.get_keyType());
	  ...
	  for (tmpEntry = currentLeafPage.getFirst(firstRid);
	       ...
	       tmpEntry = currentLeafPage.getFirst(firstRid))
	    {
	      ...
	      newLeafPage.insertRecord( tmpEntry.key,
					((TreeData)(tmpEntry.data)).getData());
	    ...
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
	      newLeafPage.insertRecord( undoEntry.key, 
					((TreeData)undoEntry.data).getData());
   ...
   *@return return a IntervalTPage instance which is pinned. 
   *        null if no key was found.
   */
  
  IntervalTPage findRunStart (KeyClass lo_key, 
			   RID startrid)
    throws IOException, 
	   IteratorException,  
	   KeyNotMatchException,
	...
	if ( trace!=null )
      ...
      pageLeaf = new IntervalTPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf=  new IntervalTPage( pinPage(pageno), headerPage.get_keyType());    
	curEntry=pageLeaf.getFirst(startrid);
      }
  ...
  private boolean NaiveDelete ( KeyClass key, RID rid)
      ...
      if ( trace!=null )
	{
	  ...
	  leafPage=new IntervalTPage(pinPage(nextpage), 
				  headerPage.get_keyType() );
	...
	leafPage=new IntervalTPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private KeyClass _Delete ( KeyClass key,
      ...
      if ( trace!=null )
	{
      ...
      if (sortPage.getType()==NodeType.NODE ) {
        ...
        PageId nextpage;
        IntervalTPage leafPage;
        leafPage=new IntervalTPage(page, headerPage.get_keyType());        
	...
	while((tmpEntry!=null) && (IntervalT.keyCompare(key,tmpEntry.key)>=0)) { 
          // WriteUpdateLog is done in the IntervalTPage level - to log the
          // deletion of the rid.
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, rid)) ) {
		...
		return null;
	      ...
	      parentPage =
		new IntervalTIndexPage(pinPage(parentPageId), 
				headerPage.get_keyType());
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  IntervalTPage nextLeafPage=new IntervalTPage(
	...
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  ...
	  if (indexPage.numberOfRecords() == 0) {
	    ...
	    childPage=new IntervalTSortedPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  // get a sibling
	  IntervalTIndexPage  parentPage;
	  parentPage=new IntervalTIndexPage(pinPage(parentPageId), 
				     headerPage.get_keyType()); 
	  ...
	  siblingPage=new IntervalTIndexPage( pinPage(siblingPageId), 
				       headerPage.get_keyType());
   ...
   *@exception UnpinPageException error when unpin a page
   */
  public IntervalTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
    throws IOException,  
	   KeyNotMatchException, 
	   IteratorException, 
	   ConstructPageException, 
    ...
    {
      IntervalTFileScan scan = new IntervalTFileScan();
      if ( headerPage.get_rootId().pid==INVALID_PAGE) {
	scan.leafPage=null;
	return scan;
      }
  ...
  void trace_children(PageId id)
	...
	if( sortedPage.getType()==NodeType.INDEX) {
	  ...
	  IntervalTPage leafPage=new IntervalTPage(sortedPage,headerPage.get_keyType()); 
