 ...
 * class IndexFileScan.  
 */
public class IntervalTFileScan  extends IndexFileScan
             implements  GlobalConst
{

  IntervalTreeFile bfile; 
  String treeFilename;     // B+ tree we're scanning 
  IntervalTPage leafPage;   // leaf page containing current record
    ...
    try {
	 ...
	 if (nextpage.pid == INVALID_PAGE) {
	    ...
	    return null;
         ...
         leafPage=new IntervalTPage(nextpage, keyType);
  ...
  public void delete_current() 
	...
	return;    
      ...
      entry=leafPage.getCurrent(curRid);  
      SystemDefs.JavabaseBM.unpinPage( leafPage.getCurPage(), false);
      bfile.Delete(entry.key, ((TreeData)entry.data).getData());
